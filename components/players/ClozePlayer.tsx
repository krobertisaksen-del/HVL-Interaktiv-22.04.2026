...
export const ClozePlayer: React.FC<PlayerProps> = ({ data, onSuccess, compact = false }) => {
  ...
  useEffect(() => {
    if (finished && score === blocks.length && onSuccess && !hasContinued) {
      setHasContinued(true);
      onSuccess({ score, total: blocks.length, mistakes });
    }
  }, [finished, score, blocks.length, onSuccess, hasContinued, mistakes]);
