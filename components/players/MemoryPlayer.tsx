    ...
    <div className="space-y-8">
        ...
        {isAllMatched && (
            <div className="text-center animate-in fade-in slide-in-from-bottom-4">
                ...
                <button onClick={() => { if(onSuccess) { onSuccess({}); setReported(true); } else setFinished(true); }} disabled={reported} className={`px-10 py-3 text-white rounded-xl font-bold hover:bg-cyan-800 shadow-lg transition-transform active:scale-95 ${reported ? 'bg-slate-300 pointer-events-none' : 'bg-cyan-700 hover:bg-cyan-700'}`}>{reported ? 'Ferdig ✓' : 'Fullført'}</button>
            </div>
        )}
    </div>
  );
