...
export const VideoPlayer: React.FC<PlayerProps> = ({ data, onSuccess }) => {
  ...
  const allResults = useMemo(() => Object.values(sceneResults), [sceneResults]);
  const totalScore = useMemo(() => allResults.reduce((acc, r: any) => acc + (r.score || 0), 0), [allResults]);
  const totalMax = useMemo(() => allResults.reduce((acc, r: any) => acc + (r.total || 0), 0), [allResults]);
  const allMistakes = useMemo(() => allResults.flatMap((r: any) => r.mistakes || []), [allResults]);
  const isPerfect = totalScore === totalMax;

  useEffect(() => {
      if (finished && isPerfect && onSuccess && !hasContinued) {
          setHasContinued(true);
          onSuccess({ score: totalScore, total: totalMax, mistakes: allMistakes });
      }
  }, [finished, isPerfect, onSuccess, hasContinued, totalScore, totalMax, allMistakes]);
