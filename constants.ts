...
export function getDefaultData(type: string) {
  switch (type) {
    ...
    case 'Bilde Hotspot': return { scenes: [{ id: 1, imageUrl: 'https://picsum.photos/id/1015/1200/800', altText: 'Fjord og fjell', hotspots: [] }] };
    ...
    case 'Dra og Slipp': return { tasks: [{ id: 1, backgroundUrl: 'https://picsum.photos/id/1018/1200/800', altText: 'Bakgrunn', items: [], zones: [] }] };
...
const SafeImage = ({ src, alt, className, style, draggable = true }) => {
    ...
    return (
      <div className={\`\${className} bg-slate-100 flex flex-col items-center justify-center p-4 text-center border-2 border-dashed border-red-300 text-red-800 text-xs overflow-hidden\`} style={style}>
        ...
        <span className="opacity-70 mt-1 text-[10px] leading-tight max-w-[200px]">
           Prøv en annen URL eller last opp bildet direkte.
        </span>
      </div>
    );
  }
  return (
    <div className="relative w-full h-full">
      {loading && <div className="absolute inset-0 bg-slate-200 animate-pulse flex items-center justify-center"><Loader2 className="animate-spin text-slate-400" /></div>}
      <img 
        src={src} 
        alt={alt || "Bilde"} 
    ...
    <div className="text-center space-y-8 animate-in fade-in py-12 bg-white rounded-xl border border-slate-100 shadow-sm h-full flex flex-col justify-center items-center max-w-2xl mx-auto">
      ...
      {mistakes.length > 0 && (
          <div className="w-full text-left bg-red-50 p-6 rounded-2xl border border-red-100 mt-4 max-w-lg">
              <h4 className="font-bold text-red-800 mb-4 flex items-center gap-2 text-sm uppercase tracking-wide"><CircleAlert size={18}/> Ting å sjå på igjen</h4>
...
const MCPlayer = ({ data, onSuccess, compact = false }) => {
  ...
  if (finished) {
    ...
    if (!onSuccess) return <CompletionScreen onRestart={reset} score={score} total={questions.length} mistakes={mistakes} message={isPerfect ? "Gratulerer!" : "Ikkje heilt..."} subMessage={isPerfect ? "Alt rett!" : "Du må ha alt rett for å fullføre."} />;
    return (
      <div className={\`text-center \${compact ? 'space-y-4 py-4' : 'space-y-6 py-8'} animate-in fade-in\`}>
        ...
        {isPerfect ? <button onClick={() => onSuccess(result)} className="px-6 py-2 bg-green-600 text-white rounded-xl font-bold">Hald fram</button> : <button onClick={reset} className="px-6 py-2 bg-slate-700 text-white rounded-xl font-bold flex items-center gap-2 mx-auto"><RotateCw size={16}/> Prøv igjen</button>}
      </div>
    );
  }
...
const ClozePlayer = ({ data, onSuccess, compact = false }) => {
  ...
  if (finished) {
    ...
    if (!onSuccess) return <CompletionScreen onRestart={reset} score={score} total={blocks.length} mistakes={mistakes} message={isPerfect ? "Gratulerer!" : "Ikkje heilt..."} subMessage={isPerfect ? "Alt rett!" : "Du må ha alt rett for å fullføre."} />;
    ...
    <div className={\`\${compact ? 'space-y-4' : 'space-y-8'} max-w-2xl mx-auto\`}>
      ...
      <div className="leading-loose text-lg text-slate-800 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          {segments.map((seg, i) => {
              ...
              const isCorrect = checked && inputs[inputIdx].trim().toLowerCase() === (seg.answer || '').trim().toLowerCase();
...
const HotspotPlayer = ({ data, onSuccess }) => {
  ...
  const isSceneFinished = useMemo(() => {
    ...
    return (currentScene.hotspots || []).every(h => viewedSpots.includes(h.id));
  ...
  if (finished) {
      ...
      return <div className="text-center animate-in fade-in"><button onClick={() => onSuccess && onSuccess({})} className="px-8 py-3 bg-green-600 text-white rounded-xl font-bold">Hald fram</button></div>;
  }
...
const DragDropPlayer = ({ data, onSuccess }) => {
  ...
  const checkAnswer = () => {
    ...
    if (correctCount === requiredCount) {
        setFeedback({ type: 'success', msg: 'Fantastisk! Alt er rett plassert.' });
    } else {
  ...
  return (
    <div className="space-y-6 select-none" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}>
      ...
      <div className="p-6 bg-slate-50 rounded-2xl border border-slate-200 min-h-[120px] flex gap-4 flex-wrap items-center justify-center">
         {items.filter(i => i.x === null).length === 0 && <span className="text-slate-400 text-sm italic">Alle element er plasserte</span>}
         {items.filter(i => i.x === null).map(i => (
         ...
         {!showResults ? (
            ...
            feedback.type === 'success' && (
                <button onClick={next} className="px-8 py-3 bg-slate-800 text-white rounded-xl hover:bg-slate-700 font-bold shadow-lg flex items-center gap-2">
                    {currentIndex < tasks.length - 1 ? 'Neste oppgåve' : 'Fullfør'} <ChevronRight size={20}/>
                </button>
            )
         )}
      </div>
    ...
    <div className="space-y-8">
        ...
        {isAllMatched && (
            <div className="text-center animate-in fade-in slide-in-from-bottom-4">
                <div className="text-green-600 font-bold text-xl mb-4">Alle par funne!</div>
  ...
  if (finished) {
      ...
      return (
          <div className="text-center space-y-6 py-8 bg-white rounded-xl border border-slate-100 animate-in fade-in">
              ...
              {isPerfect ? (
                  <button onClick={() => onSuccess({ score: totalScore, total: totalMax, mistakes: allMistakes })} className="px-8 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 shadow-lg">Hald fram</button>
              ) : (
                  <div>
                      <p className="text-red-500 font-bold mb-4 text-sm">Du må ha alt rett for å fullføre aktiviteten.</p>
...
const MixedPlayer = ({ data }) => {
  ...
  if (finished) {
      ...
      return <CompletionScreen onRestart={() => { setFinished(false); setIndex(0); setResults([]); }} score={totalScore} total={totalMax} mistakes={allMistakes} message="Gratulerer!" subMessage="Du har fullført alle deloppgåvene." />;
  }
